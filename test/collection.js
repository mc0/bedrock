(function() {
  ...
  test("clone preserves model and comparator", 2, function() {
  ...
  test("update index when id changes", 4, function() {
    ...
    var one = col.get(0);
    equal(one.get('name'), 'one', 'the name of one isnt one?');
    col.on('change:name', function (model) {
        ok(this.get(model), 'failed to get model from collection');
    });
    one.set({name: 'dalmatians', id : 101});
    equal(col.get(0), null, 'The old id still exists');
    equal(col.get(101).get('name'), 'dalmatians', 'did not return name dalmatians for new id');
  });
  ...
  test("events are unbound on remove", 2, function() {
    var counter = 0;
  ...
  test("model destroy removes from all collections", 2, function() {
  ...
  test("Colllection: non-persisted model destroy removes from all collections", 2, function() {
  ...
  test("reset", 16, function() {
    ...
    col.reset([]);
    equal(resetCount, 1);
    equal(col.length, 0, 'Col.length is now 0');
    equal(col.last(), null);
    col.reset(models);
    equal(resetCount, 2);
    equal(col.length, 4);
    ...
    col.reset();
    equal(col.length, 0, 'Col.length second is now 0');
    equal(resetCount, 4);
  ...
  test('_addReference binds all collection events & adds to the lookup hashes', 8, function() {
  ...
  test('Polymorphic models work with "advanced" constructors', 5, function () {
    ...
    equal(collection.length, 2, 'collection.length should be 2');
    ok(collection.at(0) instanceof A, 'Is at(0) instanceof A');
    equal(collection.at(0), collection.get(1), 'Does at(0) equal get(1)');
    ok(collection.at(1) instanceof B, 'Is at(1) instanceof B');
    equal(collection.at(1), collection.get(2), 'Does at(1) equal get(2)');
  });

  test("#3039: adding at index fires with correct at", 4, function() {
    var coll = new Backbone.Collection([{at: 0}, {at: 4}]);
    coll.on('add', function(model, coll, options) {
        equal(model.get('at'), options.index, "Expected " + model.get('at') + " but got " + options.index);
    });
    coll.on('adds', function(coll, models, options) {
      equal(options.index, 1);
    });
    coll.add([{at: 1}, {at: 2}, {at: 3}], {at: 1});
  });

  test("#3039: index is not sent when at is not specified", 3, function() {
    var coll = new Backbone.Collection([{at: 0}]);
    function cb(model, coll, options) {
  ...
  test('Do not add invalid models', 1, function() {
    var nothingGetsByMe = Backbone.Model.extend({
      parse: function() {
        return false;
      }
    });
    var one = new nothingGetsByMe({id: 1});
    var two = new nothingGetsByMe({id: 1});
    var collection = new Backbone.Collection([one, two]);
